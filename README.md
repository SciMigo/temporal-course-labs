...
**This repository has labs 1 and 2.** Labs 3–11 come with the full course: open a lab on
[the course site](https://scimigo.com/learn/temporal-durable-execution), choose **Download this lab**, and unzip it into this
directory. The local lab page picks it up on the next load; the terminal route works the same
way from inside the lab's folder.
...
| 03 | Determinism | break replay three ways and read each error *(full course)* |
| 04 | Activities and Failure Semantics | trigger all four timeouts; duplicate a tool call under a crash *(full course)* |
| 05 | Signals, Queries, Updates | pause across a Worker restart; make two Updates interleave *(full course)* |
| 06 | Continue-As-New and Child Workflows | one Workflow ID, many Runs; kill the child's Worker *(full course)* |
| 07 | Cancellation and Compensation | a saga that rolls back; a tool that ignores cancellation *(full course)* |
| 08 | Testing and Replay Tests | time-skipping, replay tests, failure injection, and a forensic history *(full course)* |
| 09 | Deploying Changed Workflow Code | patching, Worker Versioning, and a deploy that breaks replay *(full course)* |
| 10 | Operating Temporal in Production | read a broken execution from the UI and the CLI *(full course)* |
| 11 | Capstone: a Durable Agent Runtime | assemble it, injure it in five ways, defend the design *(full course)* |
...
Lab 2 here (and labs 8, 9 and 10 in the full course) also ships as a notebook (`lab-NN.ipynb` in each directory): they are mostly
